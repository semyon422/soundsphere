...
function GameplayView:keypressed()
	...
	local gameplayController = self.game.gameplayController
	local quickQuitMod = love.keyboard.isDown(input.quickQuitMod)
	local state = self.game.pauseModel.state
	if state == "play" then
		if kp(input.pause) and quickQuitMod then self:quit()
		elseif kp(input.pause) then gameplayController:changePlayState("pause")
		elseif kp(input.quickRestart) then gameplayController:changePlayState("retry")
		end
	elseif state == "pause" then
		if kp(input.pause) and quickQuitMod then self:quit()
		elseif kp(input.pause) then gameplayController:changePlayState("play")
		elseif kp(input.quickRestart) then gameplayController:changePlayState("retry")
		end
