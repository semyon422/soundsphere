...
---@class sea.OsuManiaV1Timings_v4: sea.ITimingValuesPreset
---@operator call: sea.OsuManiaV1Timings_v4
...
function OsuManiaV1Timings:getHeadWindows(od)
	local w = self:getBaseIntWindows(od)
	return {
		perfect = math.floor(w.perfect * 1.2) / 1000,
		great = math.floor(w.great * 1.1) / 1000,
		good = w.good / 1000,
		ok = w.ok / 1000,
		meh = w.meh / 1000,
		miss = w.miss / 1000,
...
function OsuManiaV1Timings:getTailWindows(od)
	local w = self:getBaseIntWindows(od)
	return {
		perfect = math.floor(w.perfect * 2.4) / 1000,
		great = math.floor(w.great * 2.2) / 1000,
		good = w.good * 2 / 1000,
		ok = w.ok * 2 / 1000,
		meh = w.meh / 1000,
		miss = w.miss / 1000,
...
function OsuManiaV1Timings:match(tvs)
	...
	return Timings("osumania", od), Subtimings("scorev", 1)
end
