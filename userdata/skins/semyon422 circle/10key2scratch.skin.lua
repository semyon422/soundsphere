...
noteskin:setColumns({
	offset = 0,
	align = "center",
	width = {cs, cs, cs, cs, cs, cs, cs, cs, cs, cs, cs, cs},
...
noteskin:setImagesAuto({
	...
	note_orange_00 = {"note_00", color = orange},
})
...
local columnColor = {2, 1, 0, 1, 0, 1, 1, 0, 1, 0, 1, 2}
...
local function getColor(c, column)
	...
	if c[2] and c[3] and c[4] and c[5] and c[6] then
		return ""
	end

	if column == 1 then
		-- return "_scratch"
	elseif column == 2 or column == 6 then
		return ""
...
local playfield = create_playfield(noteskin, {lanes = true, lanes_no_scratch = true})
