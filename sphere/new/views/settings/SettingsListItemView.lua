...
local SettingsListItemView = ListItemView:new()

SettingsListItemView.init = function(self)
	self:on("draw", self.draw)

	self.fontName = aquafonts.getFont(spherefonts.NotoSansRegular, 24)
end

SettingsListItemView.draw = function(self)
...
SettingsListItemView.receive = function(self, event)
	local listView = self.listView
